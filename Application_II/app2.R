...
postscript(file = "acf_app2.eps", family = "Times", horizontal=T, paper="letter")
{
...
out <- which(data$data > UCL_zaig)
out_zaig <- data$data[out]
out
out_zaig # Note there are no points out of control using the ZAIG control limits

# Create a control chart for the precipitation data

# Set the file for output and specify plot parameters
postscript(file = "cc_app2.eps", family = "Times", horizontal=T, paper="letter")
{
par(mar=c(5.5, 5.5, 2.5, 2.5)) # Margins: c(bottom, left, top, right)
par(mgp=c(3, 1, 0))

# Plot the rainfall data excluding outliers, with specified labels and axis limits
plot(data$data, xaxs = "r", pch = "+", ylab = expression("Precipitation"), 
     xlab = expression("Observations"), ylim = c(0, UCL_zaig+30), cex.axis = 1.7, cex.lab = 1.7)

# Add the outliers to the plot in red
points(out, out_zaig, pch = "+", col = "red")

# Add horizontal lines representing the upper control limits for ZAGA and ZAIG control charts
abline(h = UCL_zaga, lty = 2) # Dashed line for ZAGA
abline(h = UCL_zaig, lty = 1) # Solid line for ZAIG
...
text(20, UCL_zaig+20, labels="Phase I", cex=1.7, col=1)
text(n+25, UCL_zaig+20, labels="Phase II", cex=1.7, col=1)
}
# Close the graphics device
dev.off()
...
ZAIG_cdf <- function(x, nu_ZAIG, mu_ZAIG, sigma_ZAIG) 
{
  result=ifelse(x==0, nu_ZAIG,
                nu_ZAIG + (1 - nu_ZAIG) * ((pnorm(((x/mu_ZAIG) - 1)/(sigma_ZAIG * sqrt(x)))) 
                                           + (exp(2/(mu_ZAIG * sigma_ZAIG^2)) * pnorm((-((x/mu_ZAIG) + 1))/(sigma_ZAIG * sqrt(x))))))
  
  return(result)
}

# Set the file for output and specify plot parameters
postscript(file = "ecdf_ZAIG_app2.eps", family = "Times", horizontal=T, paper="letter")
{
...
for(i in 1:n_perturbation_vector){
  mu <- mu_ZAIG * (perturbation_vector[i]) # Multiply the mean by the perturbation factor
  
  # Calculate the probability for lower and upper limits in ZAGA
  a <- pZAIG(LCL_zaga, mu, sigma_ZAIG, nu_ZAIG) 
  b <- pZAIG(UCL_zaga, mu, sigma_ZAIG, nu_ZAIG) 
  ...
  a1 <- pZAIG(LCL_zaig, mu, sigma_ZAIG, nu_ZAIG) 
  b1 <- pZAIG(UCL_zaig, mu, sigma_ZAIG, nu_ZAIG)
...
write.table(round(results, digits = 4), 
            file="results_control_limits_app2.txt", sep=" ")
...
print(round(resulting_alpha_theoretical, digits = 4))
